...
# Modified by Fredrik Corneliusson 2004-01-29
# See AboutText for info.
...
class HistoryEntry(Entry):
    'An entry widget with history, uses up/down arrows to loop history'
    def __init__(self,*arg,**kargs):
        Entry.__init__(self,*arg,**kargs)
        self.history = []
        self.i_history = 0
        self.bind('<Key>',self.eventhandler)

    def setPageUpDownTarget(self,target):
        pass
        
    def eventhandler(self,event):
        if event.keysym in ['Up','Down']:
            if event.keysym == 'Up' and self.i_history > 1:
                self.i_history -=1
            elif event.keysym == 'Down' and self.i_history < len(self.history):
                self.i_history +=1
            if self.history:
                event.widget.delete('0', END)
                event.widget.insert(END, self.history[self.i_history-1])
            return 'break'
        
    def __add__(self,s):
        'add sting to history'
        if self.history and self.history[-1] == s: # dont add duplicates
            return self
        self.history.append(s)
        self.i_history = len(self.history)
        return self

def getLineNo(textw):
    return textw.index(INSERT).split('.')[0]

ltgray = "#%02x%02x%02x" % (240, 240, 245)
#ltltgray = "#%02x%02x%02x" % (250, 250, 250)
class TweakRecon(Frame):
    def __init__(self, parent=None):
        ...
        Label(f, text="Text:").grid(row=2, column=0, sticky=NE)
        Label(f, text="Regex:").grid(row=4, column=0, sticky=E)
        self.result = ScrolledText(f, height=10, width=60, font=font,background=ltgray)
        self.result.grid(row=0, column=1, sticky=NSEW)

        # add text styles
        self.result.tag_configure('bold', foreground='red', 
                                  font=('Courier bold', 14))
        self.result.tag_configure('help', foreground='darkblue', 
                                  font=('Courier', 12))

        fb = Frame(f)
        fb.grid(row=1, column=1, sticky=NE)

        # Line number widget
        self.res_lineno = StringVar()
        Label(fb, text='line:').pack(side=LEFT)
        Label(fb, textvariable=self.res_lineno).pack(side=LEFT)
        self.res_lineno.set(getLineNo(self.result))



        self.textEntry = ScrolledText(f, height=10, width=60, font=font,background='snow')
        self.textEntry.grid(row=2, column=1, sticky=NSEW)

        fb = Frame(f)
        fb.grid(row=3, column=1, sticky=E)

        # Line number widget
        self.text_lineno = StringVar()
        Label(fb, text='line:').pack(side=LEFT)
        Label(fb, textvariable=self.text_lineno).pack(side=LEFT)
        self.text_lineno.set(getLineNo(self.textEntry))

        # reEntry
        self.reEntry = HistoryEntry(f, width=60, font=font)
        self.reEntry.grid(row=4, column=1, sticky=EW)
        ...
        # option frame
        fo = Frame(f)
        fo.grid(row=5, column=1, sticky=W)
        self.opts = {}
        ...
        # button frame
        fb = Frame(f)
        fb.grid(row=6, column=1, sticky=W)
        ...
        Button(fb, text="About", command=self.about).pack(side=LEFT)
        Button(fb, text="Quit", command=self.quit).pack(side=LEFT)

        self.syntaxHelpWin = None
        self.flagsHelpWin = None
        self.result.insert(END, HelloText,'help')

        # - Setup Event handling -
        # Disable editing in result window
        self.result.bind('<Key>',self.resEntryEventH)
        self.result.bind('<KeyRelease>',self.resEntryEventH)
        self.result.bind('<ButtonRelease-1>',self.resEntryEventH)
        self.result.bind('<Return>',self.resEntryEventH)

        # textEntry event
        self.textEntry.bind('<Key>',self.textEntryEventH)
        self.textEntry.bind('<KeyRelease>',self.textEntryEventH)
        self.textEntry.bind('<Return>',self.textEntryEventH)
        self.textEntry.bind('<ButtonRelease-1>',self.textEntryEventH)
        # Enabel return to start eval
        self.reEntry.bind('<Return>',self.reEntryReturnEvent)


    def textEntryEventH(self,event):
        self.text_lineno.set(getLineNo(event.widget))

    def resEntryEventH(self,event):
        if event.type == '2': # key event
            # intercept edit events
            if not event.keysym in ['Left','Up','Down','Right','Next','Prior','Home','End','??']:
                return 'break'
        self.res_lineno.set(getLineNo(event.widget))

    def reEntryReturnEvent(self,event):
    ...
    def evaluate(self):
        ...
        # Make the first match pos show in result window
        if self.matchi:
            self.result.see(self.matchi[0])
            self.result.mark_set(INSERT, self.matchi[0]) # set cursor
            self.res_lineno.set(getLineNo(self.result)) # update line nr
    ...
    def reportResult(self, m, text, start):
        ...
        for b in bounds:
            ...
            if first:
                self.matchi.append(self.result.index(END))
                first = 0
            index = bindex
        if m.start() == m.end():
            if index < len(text):
    ...
    def quit(self):
        import tkMessageBox
        if tkMessageBox.askyesno("Quit", "Quit?"):
            Frame.quit(self)

    def about(self):
        import tkMessageBox
        tkMessageBox.showinfo("About TweakRecon",AboutText)
...
HelloText = \
'''Welcome to TweakRecon.

Type some text in the 'Text:' area below, then 
type a regular expression in the 'Regex:'
entry field and click on 'Evaluate' or press Return.  The
result of the match will appear here.

'''

AboutText = \
'''TweakRecon v%s is based on Recon by Brent Burley, erburley@ix.netcom.com
The tweaks was done by Fredrik Corneliusson 2004-01-28.

Tweaks:
    * Vertical scrollbars on text-widgets.
    * Show line number of cursor.
    * Result-window editing is locked.
    * Regex Entry has history (arrow up/down).
    * Pressing return in Regex entry triggers evaluation of regex.
    * Jump to (show) first match in result window.
    * Estetics: font size/color and background color.
    * And this about info :)
'''%(version)
...
if __name__ == '__main__':
    r = TweakRecon()
    r.master.title('TweakRecon - Regular Expression Test Console')
